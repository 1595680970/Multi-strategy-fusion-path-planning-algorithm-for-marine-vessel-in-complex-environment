...
def dwa(ship, goal, dt, obstacles, path):
    # 调整DWA以生成加速度和角速度
    a_samples = np.linspace(MAX_DECEL, MAX_ACCEL, 5)  # 从5个样本减少到5个
    r_samples = np.linspace(-MAX_ANGULAR_VEL, MAX_ANGULAR_VEL, 5)  # 从5个样本减少到5个
...
class Obstacle:
    def __init__(self, x, y, is_dynamic=False, size=5, shape='circle', color=(255, 255, 255), movement_type=None, direction=1, speed=MAX_SPEED):
        self.x = x
        self.y = y
        ...
        self.color = color
        if self.is_dynamic:
            self.movement_type = movement_type  # 'horizontal' 或 'vertical'
            self.direction = direction  # 1 或 -1
...
def main():
    ...
    # -------------------- 数据存储变量 --------------------
    trajectory = []  # 存储小船的位置 (x, y)
    distance_to_goal = []  # 存储每一步距离目标的距离
    u_speed = []  # 存储每一步的前向速度
    v_speed = []  # 存储每一步的横向速度
    accelerations = []  # 存储每一步的加速度a
    angular_velocities = []  # 存储每一步的角速度r
    computation_times = []  # 存储每一步的计算时间
    time_steps = []  # 存储每一步的时间
    current_time = 0.0  # 当前时间，初始化为0
    ...
    # -------------------- 路径平滑处理 --------------------
    smoothed_path = smooth_path(path, smooth_factor=100)  # 调整平滑因子为100
    logging.info(f"平滑后的路径长度: {len(smoothed_path)}")
    logging.info(f"平滑路径上的前10个点: {smoothed_path[:10]}")

    # 7. 生成动态障碍物（使用指定位置及属性）
    dynamic_obstacle_specs = [
        {'x': 211, 'y': 412, 'movement_type': 'vertical', 'direction': 1, 'speed': 8.0},
        {'x': 275, 'y': 277, 'movement_type': 'vertical', 'direction': 1, 'speed': 8.0},
        {'x': 387, 'y': 137, 'movement_type': 'horizontal', 'direction': 1, 'speed': 8.0},
        {'x': 415, 'y': 233, 'movement_type': 'vertical', 'direction': 1, 'speed': 8.0},
        {'x': 292, 'y': 222, 'movement_type': 'horizontal', 'direction': 1, 'speed': 8.0},
        {'x': 374, 'y': 173, 'movement_type': 'vertical', 'direction': 1, 'speed': 8.0},
        {'x': 395, 'y': 517, 'movement_type': 'horizontal', 'direction': 1, 'speed': 8.0},
        {'x': 202, 'y': 374, 'movement_type': 'vertical', 'direction': 1, 'speed': 8.0},
        {'x': 256, 'y': 260, 'movement_type': 'vertical', 'direction': 1, 'speed': 8.0},
        {'x': 375, 'y': 115, 'movement_type': 'horizontal', 'direction': 1, 'speed': 8.0}
    ]

    dynamic_obstacles = []
    for spec in dynamic_obstacle_specs:
        x = spec['x']
        y = spec['y']
        ...
        if (environment_static[int(y), int(x)] == 0 and
            ...
            dyn = Obstacle(x, y, is_dynamic=True, size=7, shape='triangle', color=(255, 0, 0),
                           movement_type=movement_type, direction=direction, speed=speed)
            if is_movement_range_clear(dyn, environment_static):
                ...
                logging.warning(f"动态障碍物运动范围内存在障碍，无法生成: ({x}, {y})")
    ...
    while run:
        ...
        if dist_to_goal < GOAL_THRESHOLD:
            ...
            # 正常规划
            if smoothed_path:
                computation_start = time.time()  # 记录计算开始时间
                a, r = dwa(ship, (goal[0], goal[1]), DT, obstacles, smoothed_path)
                computation_end = time.time()  # 记录计算结束时间
        ...
        # 记录小船的位置和速度
        trajectory.append((ship.x, ship.y))
        u_speed.append(ship.u)
        v_speed.append(ship.v)
    ...
    straight_distance = math.hypot(goal[0] - start[0], goal[1] - start[1])

    # 计算总到达时间
    total_time = current_time  # 以秒为单位

    # 计算平均计算时间
    if step_count > 0:
        average_computation_time = total_computation_time / step_count
    else:
        average_computation_time = 0

    # 打印或记录关键指标
    logging.info(f"总路径长度: {path_length:.2f} 像素")
    logging.info(f"直线距离: {straight_distance:.2f} 像素")
    logging.info(f"到达时间: {total_time:.2f} 秒")
    logging.info(f"平均每步计算时间: {average_computation_time:.6f} 秒")
    logging.info(f"总计算时间: {total_computation_time:.2f} 秒")

    # 生成数据图
    # 1. 距离目标的变化
    plt.figure(figsize=(10, 6))
    plt.plot(time_steps, distance_to_goal, label='Distance to Goal', color='blue')
    ...
    # 2. 加速度和角速度随时间的变化
    plt.figure(figsize=(10, 6))
    plt.plot(time_steps[:len(accelerations)], accelerations, label='Acceleration (a)', color='green')
    plt.plot(time_steps[:len(angular_velocities)], angular_velocities, label='Angular Velocity (r)', color='red')
    plt.xlabel('Time (seconds)')
    plt.ylabel('Control Input')
    plt.title('Control Inputs Over Time')
    plt.legend()
    plt.grid(True)
    plt.savefig('control_inputs_over_time.png')
    plt.close()

    # 3. 前向速度和横向速度随时间的变化
    plt.figure(figsize=(10, 6))
    plt.plot(time_steps, u_speed, label='Forward Speed (u)', color='orange')
    plt.plot(time_steps, v_speed, label='Lateral Speed (v)', color='purple')
    plt.xlabel('Time (seconds)')
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(traj_x, accelerations, label='Acceleration (a)', color='green', s=10, alpha=0.5)
    plt.scatter(traj_x, angular_velocities, label='Angular Velocity (r)', color='red', s=10, alpha=0.5)
    plt.xlabel('X Position (pixels)')
    plt.ylabel('Control Input')
    plt.title('Control Inputs Over X Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('control_inputs_over_position_x.png')
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.scatter(traj_y, accelerations, label='Acceleration (a)', color='green', s=10, alpha=0.5)
    plt.scatter(traj_y, angular_velocities, label='Angular Velocity (r)', color='red', s=10, alpha=0.5)
    plt.xlabel('Y Position (pixels)')
    plt.ylabel('Control Input')
    plt.title('Control Inputs Over Y Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('control_inputs_over_position_y.png')
    plt.close()

    # 5. 速度随位置的变化
    plt.figure(figsize=(10, 6))
    plt.scatter(traj_x, u_speed, label='Forward Speed (u)', color='orange', s=10, alpha=0.5)
    plt.scatter(traj_x, v_speed, label='Lateral Speed (v)', color='purple', s=10, alpha=0.5)
    plt.xlabel('X Position (pixels)')
    plt.ylabel('Speed (pixels/s)')
    plt.title('Ship Speed Over X Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('speed_over_position_x.png')
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.scatter(traj_y, u_speed, label='Forward Speed (u)', color='orange', s=10, alpha=0.5)
    plt.scatter(traj_y, v_speed, label='Lateral Speed (v)', color='purple', s=10, alpha=0.5)
    plt.xlabel('Y Position (pixels)')
    plt.ylabel('Speed (pixels/s)')
    plt.title('Ship Speed Over Y Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('speed_over_position_y.png')
    plt.close()

    # 6. 轨迹图（可选）
    plt.figure(figsize=(10, 8))
    plt.imshow(environment_static, cmap='gray', origin='lower')
    plt.plot(traj_x, traj_y, color='yellow', label='Trajectory')
    plt.scatter(start[0], start[1], color='green', marker='*', s=200, label='Start')
